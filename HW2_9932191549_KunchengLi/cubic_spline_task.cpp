...
//  target[L_EB].th  += 0.5;
    ...
    ctarget[LEFT_HAND].x[_X_] = cart_des_state[LEFT_HAND].x[_X_];
    ctarget[LEFT_HAND].x[_Y_] = cart_des_state[LEFT_HAND].x[_Y_];
    ctarget[LEFT_HAND].x[_Z_] = cart_des_state[LEFT_HAND].x[_Z_];
...
run_cubic_spline_task(void)
{
 ...
 double sum=0;
    ...
    if (tau <= -0.5 ) {
        if (my_des == 0) {
            ctarget[LEFT_HAND].x[_Z_] -= 0.03;
            tau = 1;
            my_des = 1;
        } else if (my_des == 1) {
            ctarget[LEFT_HAND].x[_X_] += 0.04;
            tau = 1;
            my_des = 2;
        } else if (my_des == 2) {
            ctarget[LEFT_HAND].x[_Z_] += 0.03;
